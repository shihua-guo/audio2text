...
class Mp3ToTextRuntimeTests(unittest.TestCase):
    ...
    def test_deduplicates_adjacent_alignment_segments_with_same_timestamps(self):
        repeated_text = "这一段字幕只应该出现一次，重复的对齐结果需要被过滤掉"
    ...
    def test_collapses_internal_repetition_in_alignment_segments(self):
        ...
        class FakeEngine:
            def asr(self, **kwargs):
                ...
                return types.SimpleNamespace(
                    text=repeated_text,
                    alignment=types.SimpleNamespace(items=[item]),
                )
        ...
        self.assertEqual(len(segments), 1)
        self.assertEqual(segments[0].text, "事儿啊，你说这")
        self.assertEqual(segments[0].start, 2700.0)
        self.assertAlmostEqual(segments[0].duration, 0.1)
